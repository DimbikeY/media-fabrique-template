---
type: telegraph-longread-draft
status: draft (live URL exists, awaiting DD final version)
published-url: TODO
telegraph-author: "DD"
author: DD
source-doc: docs/longread-ru.md (worktree public-release @ e39fe3b, commit a69f094)
created: 2026-07-26
last-touched: 2026-07-26
editing-conventions:
  - Markdown, not Telegraph DOM JSON. Telegraph Node DOM is generated by the script `scripts/md_to_telegraph.py` at publish time.
  - Free to edit the text, remove/add sections.
  - Code blocks (```bash / ```python / ```cron) are preserved as `<pre><code>` in Telegraph.
  - Inline links `[text](url)` are preserved as `<a href>`.
  - Markdown `**bold**` / `*italic*` are NOT supported by the converter (they will remain as literal `**bold**` in Telegraph). If bold is needed — wrap the asterisks as `**word**`, but visually they will remain as text. Alternative: use CAPS or emoji (⚡ ❗ ⚠️) instead of bold.
  - Inline code `` `code` `` is preserved as `<code>`.
  - Images `![alt](url)` are preserved as `<img src=url>`. The URL must be public (https), otherwise Telegraph will not load it.
submit-process:
  - Open this file, edit the markdown as desired.
  - When ready — send the final markdown (via webchat, or write to this file and say "done").
  - The pipeline will run `scripts/md_to_telegraph.py` on the final output, produce a new Telegraph Node DOM, and use `scripts/publish_telegraph.py` to do editPage (update the current page) or republish (new page, URL changes).
  - After editPage/republish — update the README cross-link if the URL changes.
related-files:
  - docs/longread-ru.md — public copy in the worktree (what gets published)
  - docs/longread-telegraph-ru.json — Telegraph Node DOM (137 nodes, 11 h3, 79 p, 20 ul, 16 pre, 11 hr)
  - scripts/md_to_telegraph.py — converter
  - scripts/publish_telegraph.py — publisher (createAccount + createPage)
---
